...
impl Space {
    ...
    unsafe fn read_handler<'trie, 'path>(
        ...
        py: *mut PySessionManager,
        request: ResourceRequest,
    ) -> Resource<'trie, 'path> {
        match request {
            ResourceRequest::BTM(prefix) => {
                Resource::BTM(btm.as_ref().unwrap().read_zipper_at_path(prefix.as_ref()))
            }
            ResourceRequest::ACT(name) => {
                let act = mmaps
                    .as_mut()
                    .unwrap()
                    .entry(OwnedSourceItem::from(name.as_ref()))
                    .or_insert_with(|| {
                        trace!(target: "query_multi_i", "open new ACT {}", name.as_ref());
                        ArenaCompactTree::open_mmap(format!("{ACT_PATH}{}.act", name.as_ref()))
                            .unwrap()
                    });
                trace!(target: "query_multi_i", "taking RZ of {}", name.as_ref());
            ...
            ResourceRequest::Z3(instance) => {
                ...
                let mut z3 = z3s
                    .as_mut()
                    .unwrap()
                    .get_mut(&OwnedSourceItem::from(instance.as_ref()))
                    .unwrap_or_else(|| panic!("non existent z3 {}", instance.as_ref()));
                z3.stdin
                    .as_mut()
            ...
            ResourceRequest::PY(session, query) => {
                trace!(target: "query_multi_i", "PY resource requested for {}", session.as_ref());
                let result = (*py)
                    .execute_source(session.as_ref(), &query)
                    .unwrap_or_else(|e| panic!("PY source failed: {e}"));
                let bytes = py_result_to_expr_bytes(&result)
                    .unwrap_or_else(|e| panic!("PY result serialization failed: {e}"));
                let mut values = PathMap::new();
                values.insert(&bytes, ());
                Resource::PY(values.into_read_zipper(&[]))
            }
        }
    }
        ...
        'w: 'a,
    {
        match request {
            WriteResourceRequest::BTM(p) => {
                ...
                wzs.push(zh.write_zipper_at_exclusive_path_unchecked(p.as_ref()));
            ...
            WriteResourceRequest::Z3(f) => {
                ...
                let instance = z3s
                    .as_mut()
                    .unwrap()
                    .entry(OwnedSourceItem::from(f.as_ref()))
                    .or_insert_with(|| {
            ...
            WriteResourceRequest::PY(session, _) => {
                trace!(target: "transform", "retrieving PY session {}", session.as_ref());
                WriteResource::PY(py)
            }
        }
    }
    ...
    pub fn query_multi_i<F: FnMut(Result<&[u32], BTreeMap<(u8, u8), ExprEnv>>, Expr) -> bool>(
        ...
        for e in pat_args[1..].iter() {
            ...
            factors.push(
                src.source(
                    src.request()
                        .map(|request| unsafe { Self::read_handler(btm, mmaps, z3s, py, request) }),
                ),
            );
            srcs.push(src);
        }
    ...
    pub fn transform_multi_multi_i(
        ...
        let touched = Self::query_multi_i(
            ...
            &read_copy,
            pat_expr,
            (&mut self.py as *mut PySessionManager),
