use crate::python::{PyCommand, py_command_from_expr};
    ...
    pub(crate) fn pjoin(&self, other: &Self) -> Option<Self> {
        match self {
            WriteResourceRequest::BTM(s) => {
                match other {
                    WriteResourceRequest::BTM(o) => {
                        ...
                        Some(WriteResourceRequest::BTM(Cow::Owned(
                            s.as_ref()
                                [..pathmap::utils::find_prefix_overlap(s.as_ref(), o.as_ref())]
                                .to_vec(),
                        )))
                    }
                    _ => None,
            ...
            WriteResourceRequest::ACT(s) => match other {
                WriteResourceRequest::ACT(o) if s == o => {
                    Some(WriteResourceRequest::ACT(s.clone()))
                }
                _ => None,
            },
            WriteResourceRequest::Z3(s) => match other {
                WriteResourceRequest::Z3(o) if s == o => Some(WriteResourceRequest::Z3(s.clone())),
                _ => None,
            },
            WriteResourceRequest::PY(s, command) => match other {
                WriteResourceRequest::PY(o, _) if s == o => {
                    Some(WriteResourceRequest::PY(s.clone(), command.clone()))
                }
                _ => None,
            },
        }
...
impl PartialOrd for WriteResourceRequest {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match self {
            ...
            WriteResourceRequest::PY(s, _) => {
                if let WriteResourceRequest::PY(o, _) = other {
                    if s == o { Some(Ordering::Equal) } else { None }
                } else {
                    None
                }
...
impl Sink for CompatSink {
    ...
    fn request(&self) -> impl Iterator<Item = WriteResourceRequest> {
        ...
        std::iter::once(WriteResourceRequest::BTM(Cow::Borrowed(p)))
    }
    fn sink<'w, 'a, 'k, It: Iterator<Item = WriteResource<'w, 'a, 'k>>>(
        &mut self,
        mut it: It,
...
impl Sink for ACTSink {
    ...
    fn request(&self) -> impl Iterator<Item = WriteResourceRequest> {
        ...
        std::iter::once(WriteResourceRequest::ACT(Cow::Borrowed(self.file)))
    }
    fn sink<'w, 'a, 'k, It: Iterator<Item = WriteResource<'w, 'a, 'k>>>(
        &mut self,
        mut it: It,
...
impl Sink for Z3Sink {
    ...
    fn request(&self) -> impl Iterator<Item = WriteResourceRequest> {
        return std::iter::once(WriteResourceRequest::Z3(Cow::Borrowed(self.ins)));
    }
    fn sink<'w, 'a, 'k, It: Iterator<Item = WriteResource<'w, 'a, 'k>>>(
        &mut self,
        mut it: It,
...
impl Sink for PySink {
    fn new(e: Expr) -> Self {
        destruct!(e, ("PY" {session: &str} {command: Expr}), {
            return PySink { e, session: session.to_string(), command: py_command_from_expr(command).expect("invalid PY command"), executed: false }
        }, _err => { panic!("py not the right shape") });
    }
    fn request(&self) -> impl Iterator<Item = WriteResourceRequest> {
        std::iter::once(WriteResourceRequest::PY(
        ...
        'k: 'w,
    {
        ...
        let WriteResource::PY(manager) = it.next().unwrap() else {
            unreachable!()
        };
        unsafe {
            (*manager)
                .execute_sink(&self.session, self.command.clone())
                .expect("Python sink failed")
        };
        self.executed = true;
    }
    fn finalize<'w, 'a, 'k, It: Iterator<Item = WriteResource<'w, 'a, 'k>>>(
    ...
    fn new(e: Expr) -> Self {
        if unsafe {
            ...
            *e.ptr == item_byte(Tag::Arity(3))
            ...
            return ASink::PySink(PySink::new(e));
        } else if unsafe {
            *e.ptr == item_byte(Tag::Arity(3))
                && *e.ptr.offset(1) == item_byte(Tag::SymbolSize(4))
                && *e.ptr.offset(2) == b'w'
